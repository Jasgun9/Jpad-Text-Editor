...
main_application.geometry('909x697')
main_application.title('Jpad')
...
font_size['value'] = tuple(range(8, 82, 2))
font_size.current(4)
...
align_left_btn.grid(row=0, column=6, pady=5)
...
align_right_btn.grid(row=0, column=8, pady=5)
...
def new_file(event=None):
    global url
    url = ''
    main_application.title('Untitled - Jpad text editor')
    text_editor.delete(1.0, tk.END)
...
def open_file(event=None):
    ...
    url = filedialog.askopenfilename(initialdir=os.getcwd(), title='Select file',defaultextension=".txt", filetypes=(('Text File', '*.txt'), ('All Files', '*.*')))
    try:
        ...
        return
    main_application.title(os.path.basename(url) + " - Jpad text editor")
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            message = messagebox.askyesnocancel('Warning', 'Do you want to save the file ?')
            if message is True:
                if url:
...
def find_fun(event=None):
    ...
    find_dialog.iconbitmap('icon.ico')
    ...
    def replace():
        ...
        text_editor.delete(1.0, tk.END)
        text_editor.insert(new_content)
    ...
    #--#button grid
    find_button.grid(row=2,column=0, pady=4,padx=8)
    replace_button.grid(row=2,column=1, pady=4,padx=8)
...
#--showing statusbar functionality
def hide_statusbar():
...
file.add_command(label="Save as", image=save_as_icon, compound=tk.LEFT,
...
view.add_checkbutton(label="Tool Bar",onvalue=1,offvalue=0,variable = show_toolbar, image=tool_bar_icon, compound=tk.LEFT,command=hide_toolbar)
...
def data_json():
    ...
    col = string.punctuation[1]
    cu = ['{', '}']
    c1 = cu[0]
    c2 = cu[1]
    # bg = text_editor['background']
    # fg = text_editor['fg']
    default = f'{c1}\n\t"fg":"#000000",\n\t"bg":"#ffffff"\n{c2}'
    file.write(default)
    file.close()
...
opening = open('data.json', 'r')
data_read = opening.read()
if data_read == "":
    data_json()
else:
    pass
...
def theme():
    with open('data.json', 'r') as file:
        ...
        if file_read == '':
            data_json()
            # messagebox.showinfo('error', 'please add the color to data.json')
        else:
            data = json.load(file)
            bg_color = data['bg']
            fg_color = data['fg']
...
color_theme.add_command(label = 'Theme' ,command=theme)
